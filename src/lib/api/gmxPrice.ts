...
export function parseGmxUsdValue(rawValue: string, tokenDecimals: number): USD {
  ...
  try {
    ...
    const fractionalPart = fractionalDivisor > BigInt(0)
    ...
    if (!Number.isFinite(result)) {
      console.warn(`[GmxPrice] Non-finite result for ${rawValue} / 10^${GMX_USD_DECIMALS - tokenDecimals}`);
      return usd(0);
    }
 ...
 */
export function parseGmxAnnualRate(rawRate: string, tokenDecimals: number = 30): Percent {
  ...
  try {
    ...
    const annualized = (bigRate * SECONDS_PER_YEAR * BigInt(100) * precisionMultiplier) / divisor;
 ...
 */
export function parseGmxPerSecondRate(rawRate: string, tokenDecimals: number = 30): number {
 ...
 */
export function calculatePriceChangePercent(currentPrice: Price, previousPrice: Price): Percent {
 ...
 */
export function calculateSpreadPercent(minPrice: Price, maxPrice: Price): Percent {
